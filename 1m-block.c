...
char host[100]; // Assume that size of size is less than 100
...
static uint32_t print_pkt (struct nfq_data *tb)
{
	...
	if (ret >= 0){
		...
		struct libnet_ipv4_hdr* ipv4_hdr = data;
		ipv4_len = 20;
		...
		if(ipv4_hdr->ip_p == IPTYPE_TCP){
			...
			if(data_len > 0){
				...
				if(strncmp(http, "GET", 3) == 0 || strncmp(http, "POST", 4) == 0){
					...
					host_len = strcspn(host_ptr, "\n");
					strncpy(host, host_ptr, host_len-1);
					// *(host+host_len) = NULL; // Null Termination
					
					// Resolving arbitrarily added "www."
					if(strncmp(host, "www.", 4) == 0){
					
						strncpy(host, host+4, sizeof(host)-4);
					
					}
					
					char* dot = strstr(host, ".");
					*dot = NULL;
					dot += 1;
					
					if(dict_has(dict, dot) == 0){
						verdict = NF_ACCEPT;
					}
					else{

						struct Trie* head = dict_get(dict, dot);
						
						verdict = trie_find(head, host) == 0 ? NF_ACCEPT : NF_DROP;
					
						if(verdict == NF_DROP) printf("\n\n!!!!!!!!!! Target Detected !!!!!!!!!!\nHost: %s.%s\n", host, dot);
